...
export const fetchData = async (id='restore') => {
  const doc = await spreadsheet(id);
  ...
  const sheetsData = sheetList.map(async ([ sheetTitle, sheetSlug]) => {
    ...
    const refinedData = rawData.map((row) => {
      ...
      let sheetHasUrl = false;

      entryList.forEach((entry) => {
        if(Object.keys(entry)[0].match(/url/gi)) {
          sheetHasUrl = true;
        }
      });

      if(!sheetHasUrl) {
        entryList.push({ URL: `https://www.cf88.com.br/stf/${sheetSlug}/${pageSlug}` });
      }
